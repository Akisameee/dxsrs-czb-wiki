...
- 逆向 `re/dump/*`、存档样本、session jsonl、日志或其他大文件时，禁止一次性 `Get-Content` 整个文件。
- 优先用 `rg` / `Select-String` 精确查找类名、方法名、字段名或关键字符串，再用 `Select-Object -Skip ... -First ...` 小块读取。
- 读取目录时避免展开超大树；先用 `rg --files` 配合路径或文件名过滤，只列出当前问题需要的文件。
- 工具输出要控制在可读范围内；如果命令可能产生大量输出，必须先加过滤条件、`-TotalCount`、`-First` 或等价限制。
- 恢复 Codex 历史会话时，只抽取相关用户/助手文本、关键命令和补丁摘要，不要整段倒出 transcript，也不要读取 `auth.json` 等敏感文件。
...
- 对于字段集合固定、后续需要频繁微调顺序或展示方式的详情区块，优先显式写出每个字段，不要用 `v-for` 循环渲染。
...
- 不要主动运行页面构建；如果页面有问题，用户会反馈。
- 只有在用户明确要求，或用户反馈了需要构建定位的页面问题时，才运行 `npm run build`。
- 在当前 Windows 工作区里，`npm run build` 应尽量在沙箱外执行；普通沙箱执行可能因为 Vite 子进程启动报 `spawn EPERM`。
- 数据生成、脚本输出等和当前改动直接相关的轻量验证可以按需运行。
